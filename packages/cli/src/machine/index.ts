...
export default class Machine {
  ...
  constructor({ ipfs = DEFAULTS.IPFS, ethereum = DEFAULTS.ETHEREUM, mnemonic = DEFAULTS.MNEMONIC, address = DEFAULTS.ADDRESS, devices = DEFAULTS.DEVICES }: { ipfs?: string; ethereum?: string; mnemonic?: string; address?: string; devices?: boolean } = {}) {
    // IPFS
    ...
    // utils
    this.fs    = fs
    this.paths = { root: path.join(os.homedir(), '.chaos'), tracks: path.join(os.homedir(), '.chaos', 'tracks'), log: path.join(os.homedir(), '.chaos', 'log') }
    ...
    if (devices) {
      ...
      this.lcd!.on('ready', () => {
        ...
        this.lcd!.write('Welcome chaos', err => { this.log.error(err.toString()) })
      })
